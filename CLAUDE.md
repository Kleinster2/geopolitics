...
- Table-safe wikilinks: in Markdown table cells, avoid raw `[[Note|alias]]`; the `|` creates extra columns. Prefer `[[Note]]`, or escape as `[[Note\|alias]]`. For table-heavy edits, run `rg -n "\|.*\[\[[^\]\\]+\|" .` before finishing.
...
- **[[2026 Iran conflict]] / [[Strait of Hormuz]] (May 29 update)** — US and Iranian negotiators have a tentative ceasefire-extension / Hormuz-reopening framework, but the operating terms remain contested. US-side reporting describes unrestricted reopening, mine removal, no tolls, and gradual pressure relief; Iran-side reporting points to reopening under Tehran's own arrangements with traffic regulation and fees still in view. Brent has fallen sharply on deal hopes but remains around $91, while traffic through Hormuz is still a small fraction of pre-conflict levels. [[Robert Pape]]'s May 27 Escalation Trap preview frames this as the core strategic test: reopening and normalization are different states if markets continue pricing Gulf passage as structurally dangerous.
